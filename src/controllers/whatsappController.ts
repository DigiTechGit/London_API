...
export default function whatsappRoutes(
  ...
  const io = socketIo(fastify.server, {
    cors: {
      origin: "https://envioprime.com.br" // produção
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("qr", (qr: string) => {
      console.log("\x1b[33m%s\x1b[0m", "QR Code recebido do bot.");
      latestQrCode = qr;
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!motorista.telefone) {
        return reply
          .status(400)
          .send({ error: "Telefone do motorista não cadastrado" });
      }
      ...
      io.emit("sendFile", {
        ...
        fileName: nmArquivo,
        caption:
          "Segue em anexo a planilha com os endereços dos destinatários.",
      });
